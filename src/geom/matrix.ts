...
export function matrixCreate(): Matrix {
	return {
		a: 1,
		b: 0,
		c: 0,
...
export function matrixConcat(matrix0: Matrix, matrix1: Matrix, result: Matrix) {
...
export function matrixGetScale(matrix: Matrix): number {
	return mathMax(
		mathAbs(matrix.a),
		mathAbs(matrix.b),
		mathAbs(matrix.c),
...
export function matrixTransformPoint(matrix: Matrix, point: Point, result: Point) {
	const { x, y } = point;
	result.x = x * matrix.a + y * matrix.c;
	result.y = x * matrix.b + y * matrix.d;
}

export function matrixGetDeterminant(matrix: Matrix): number {
...
export function matrixTransformInversePoint(matrix: Matrix, point: Point, result: Point) {
	let determinant = matrixGetDeterminant(matrix);
